...
repair_line() {
  ...
  case "$HARNESS" in
    ...
    codex)
      ...
      ;;
    copilot)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with one standalone asynchronous Bash call running bin/fm-watch-arm.sh.'
      ;;
    pi|pi-signed)
...
ordinary_wake_line() {
  case "$HARNESS" in
    ...
    codex)
      ...
      ;;
    copilot)
      printf '%s\n' '- Ordinary wake: start the next standalone asynchronous Bash bin/fm-watch-arm.sh call after handling the notification.'
      ;;
    pi|pi-signed)
