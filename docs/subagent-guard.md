...
| GitHub Copilot CLI | Built-in `task` plus custom-agent tools | Scoped guard wired through the repository-wide `preToolUse` hook for local sessions; the adapter stays inert under GitHub Actions so Copilot cloud-agent jobs retain their worker delegation tools. |
